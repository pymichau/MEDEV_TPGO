...
Pierre* Joueur::jouer(Plateau& goban)
{
    ...
    while (fincoup==NULL)
    {
        while (abscisse!='A' || abscisse!='B' || abscisse!='C' || abscisse!='D' || abscisse!='E' || abscisse!='F' || abscisse!='G' || abscisse!='H' || abscisse!='I' || abscisse!='J' || abscisse!='a' || abscisse!='b' || abscisse!='c' || abscisse!='d' || abscisse!='e' || abscisse!='f' || abscisse!='g' || abscisse!='h' || abscisse!='i' || abscisse!='j')
            {//la condition serait plus propre si je trouvais le moyen de dire abscisse "n'appartiens pas à l'ensemble"
                ...
                if (goban.getSize()==5)//si l'utilisateur tape une lettre de G à J et que le tableau est de 5*5, la saisie est incorrecte, cette boucle permet d'en tenir compte
                   {
                        if (abscisse!='A' || abscisse!='B' || abscisse!='C' || abscisse!='D' || abscisse!='E' || abscisse!='F' || abscisse!='a' || abscisse!='b' || abscisse!='c' || abscisse!='d' || abscisse!='e' || abscisse!='f')
                            {
                                abscisse='z';
                            }
                    }
